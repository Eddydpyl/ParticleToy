...
import static physics.LinearSolver.vecDiff;
import static physics.LinearSolver.vecDot;
import static physics.LinearSolver.vecTimesScalar;

public class AngularSpringForce2D implements Force {

	protected double ks,kd; 
    protected double rad; 
    private Particle p1,p2,p3;


    public AngularSpringForce2D(Particle par1, Particle par2, Particle par3, double ks, double kd, double angle){
    ...
    void apply(){
        ...
        p2.setForces(vecAdd(p2.getForces(), vec1));
        p3.setForces(vecAdd(p3.getForces(), vec2)); 
        p1.setForces(vecDiff(p1.getForces(), vec0)); 
    }
